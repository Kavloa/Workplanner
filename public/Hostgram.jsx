    ...
    function groupConsecutiveAssignments(assignments = []) {
      ...
      return groupedTasks.map((group, index) => {
        ...
        return {
          ...
          assignments: group,
          css: isFirstOccurrence ? "sqrend" : "", // Add your CSS class or property here
        };
      });
    }
        ...
        return data.flatMap((dataItem) => {
          ...
          return updatedItems
            .map((group) => {
              ...
              const task = {
                id: group.group_id,
                text: group.task_name + " (" + group.part_id + ")",
...
// console.log(projects[group.employee]);
...
// Define the substring to search for in the data-task-id
      ...
      element.tasks.forEach((task) => {
        if (task.start_date) {
          ...
          sizes.forEach((sizeElement) => {
            const topValue =
              sizeElement.top < 0
                ? Math.max(sizeElement.top, 0) + 35
                : sizeElement.top;
            if (sizeElement.project === task.project) {
              const rule = `.myDynamicClass${task.id} {border: 1px solid black !important; width:30px !important; position: absolute; top: ${topValue}px; left: ${sizeElement.left}px; height: 25px !important; background-color: ${task.color}; font-weight: bold; z-index:20; }`;
              const rule2 = `.myDynamicClass${task.id} {border: 1px solid red !important; width:30px !important; position: absolute; top: ${topValue}px; left: ${sizeElement.left}px; height: 25px !important; background-color: ${task.color}; font-weight: bold; z-index:20; }`;
          ...
          if (localStorage.getItem("taskname") === task.Ptask) {
            ...
            const marker = {
              id: task.id,
              start_date: markerStartDate,
              css: `myDynamicClass${task.id} hiden`,
              text: task.text,
              project: task.project,
            };
            gantt.addMarker(marker);
  ...
  useEffect(() => {
    ...
    const employeesData = emp.map((employee) => {
      ...
      const groupedByMonth = availabilities.reduce((result, availability) => {
        const date = new Date(availability.date); // Assuming each availability has a `date` property
        ...
        if (availability.total_ava !== "00:00:00") {
          result[key].push(date); // Push the full Date object
        }
        return result;
      }, {});
      ...
      Object.entries(groupedByMonth).forEach(([monthKey, availableDates]) => {
        ...
        const availableDatesSet = new Set(
          availableDates.map((date) => date.toISOString().split("T")[0])
        );
